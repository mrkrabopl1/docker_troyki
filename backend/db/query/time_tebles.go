...
const (
	// createTempTableSQL – создание временной таблицы с товарами по фильтрам
	CreateTempTableSQL = `
	...
	// Индексы на временной таблице
	CreateIndexIDSQL    = `CREATE INDEX ON tmp_product_ids(id)`
	CreateIndexBrandSQL = `CREATE INDEX ON tmp_product_ids(brand_id)`
	CreateIndexLineSQL  = `CREATE INDEX ON tmp_product_ids(line_id)`
)

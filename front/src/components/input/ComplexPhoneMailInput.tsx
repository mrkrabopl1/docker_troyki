...
const ComplexPhoneMailInput: React.FC<propsRowType> = (props) => {
    const inputRef = useRef<HTMLInputElement>(null)
    let { onBlur, onChange, onFocus, className, val } = { ...props }
    const [phoneVal, setPhoneVal] = useState<string>(val ? val : "")
    const cursorPositionRef = useRef<number>(2)
    let isPhone = useRef<boolean>(true);
    ...
    const numberValidation = (val: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        if (val.key === "Backspace") {
            ...
            if (isPhone.current) {
                ...
                let finalStr = phoneVal.slice(0, cursorPosition - 1);
                if(isFinite(Number(finalStr)) && (finalStr[0] === "7" || finalStr[0]=== "8") && finalStr.length < 11){
                    isPhone.current = true
                    let str = removeChar("+"+finalStr)
                    setPhoneVal(str)
                    onChange(str)
                }else{
                    setPhoneVal(finalStr)
        ...
        if (isFinite(num)) {
            ...
            let finalStr = phoneVal
            if (isPhone.current) {
                isPhone.current = false;
                finalStr = (phoneVal.replace(/[+,-,\s]/g, ""));
            }
            finalStr += val.key;
            cursorPositionRef.current = cursorPositionRef.current + 1
            setPhoneVal(finalStr);
            onChange(finalStr);
        }
    }
...
export default ComplexPhoneMailInput
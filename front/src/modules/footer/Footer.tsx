...
const NavLinkWrapper: React.FC<{ href: string; children: React.ReactNode }> = ({ href, children }) => {
    const router = useRouter();
    const isActive = router.pathname === href;

    return (
        <Link href={href} className={isActive ? s.linkActive : s.link}>
            {children}
        </Link>
    );
};
        ...
        <footer className={s.footer}>
            <div className={s.container}>
                <div className={s.grid}>
                    <ColumnWithChilds header="Помощь" headerClassName={s.columnHeader} className={s.column} rows={helpLinks} />
                    <ColumnWithChilds header="Информация" headerClassName={s.columnHeader} className={s.column} rows={policyLinks} />
                    <ColumnWithChilds header="О нас" headerClassName={s.columnHeader} className={s.column} rows={aboutLinks} />
                    ...
                    <div className={s.rightColumn}>
                        ...
                        <div className={s.socials}>
                            {socials.map(({ Icon, name, url }, i) => (
                                <a key={i} href={url} className={s.socialLink} target="_blank" rel="noopener noreferrer" aria-label={name}>
